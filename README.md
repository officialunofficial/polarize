...
cleanly, and the server has been driven end to end over stdio on a real
macOS session — `initialize`, `tools/list`, and a `tools/call` for each
of the four tools all round-trip real JSON-RPC. What has **not** been
verified on this machine, because it lacks a granted Screen Recording /
Accessibility TCC authorization, is a tool call actually *succeeding*
against a real screen/app — every call observed so far correctly stops
at the permission boundary (see "Permissions" below) rather than
completing. A human on a macOS session with both permissions granted
still needs to confirm a `screenshot` returns real pixels, `describe`
returns a real AX tree, and `tap`/`keyboard` visibly land — see
...
Without Accessibility, `describe`, `tap`, and `keyboard` fail with a
clean, structured permission error (`AXIsProcessTrusted` /
`CGPreflightPostEventAccess` are checked before any native call runs).
`screenshot` does not yet have an equivalent Screen Recording preflight
check — without that permission it currently fails with
`ScreenCaptureKit`'s own raw error message instead of the same clean
shape; see `apps/polarize/src/server.rs`'s module doc comment for the
detail. Either way, no tool silently does nothing.
...
- `crates/polarize-macos` — real macOS framework bindings implementing
  ...
  enumeration). Accessibility-tree inspection is **not** built on
  `objc2-accessibility` despite that crate's name: it binds Apple's
  newer, unrelated content-authoring "Accessibility" framework
  (`AXCustomContent`, `AXChart`, …), not the classic `AXUIElement`
  inspection API every screen reader and UI-automation tool actually
  walks, and no `objc2-application-services` crate exists in the `objc2`
  umbrella either. `polarize-macos` fills that gap with a small,
  hand-written `extern "C"` binding to `ApplicationServices` directly
  (`src/ax_ffi.rs`) — see that file's doc comment for the full
  reasoning. macOS-only. Its real native-API behavior cannot be
...
from the system's dyld shared cache with no help needed; on the affected
machine it did not, and the binary needed the Swift 5.5 back-deployment
concurrency library made available at run time, e.g.:
...
This runs `polarize-core`'s full unit-test suite (55 tests covering
...
factors out of its native calls (24 tests covering app-identity
