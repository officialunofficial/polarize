...
// symbol resolution in this crate. That is `ax_ffi`, `skylight_ffi`,
// `disclaimed_spawn` — rather than shared. Each is a plain, zero-cost
// `libSystem` link declaration, not real duplicated logic.
unsafe extern "C" {
...
unsafe fn wait_for_child_exit_code(pid: i32) -> Result<i32, String> {
    loop {
        ...
        if outcome == -1 {
            // SAFETY: `__error()` is the documented Darwin way to
            // read `errno` from a plain FFI call site. This runs
            // right after the call that may have set it.
